...
sub pushTopicContext {
    ...
    foreach (@webPath) {
        ...
        $subWeb .= $_;
        $back = $this->_getBackend( $subWeb, $Foswiki::cfg{WebPrefsTopicName} );
        if($back->can('inheritsFrom')) {
            my $inherit = $back->inheritsFrom();
            while($inherit) {
                $inherit =~ s#\s*$##;
                my $iBackend = $this->_getBackend( $inherit, $Foswiki::cfg{WebPrefsTopicName} );
                $stack->newLevel( $iBackend );
                $inherit = $iBackend->inheritsFrom();
            }
        }
        $stack->newLevel($back);
    }
